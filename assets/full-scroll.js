...
(function($) {
	$.fn.fullpage = function(options) {
		...
		*/
        ...
        //add first and last classes for all-way scrolling
		$(options.sectionSelector).each(function(){
			$(this).find('.fp-slide:first').addClass('fp-first');
			$(this).find('.fp-slide:last').addClass('fp-last');
		});
		...
		*/
		function scrolling(type, scrollable, destiny, slide){
			if (!isScrollAllowed[type]){
				return;
			}
			
			var check, scrollSection, section;
            
            //if active section has slides - lets slide horizontally through all the slides, then back up or down
            if( $('.section.active').has('.fp-slide').length ){
    			
    			var activeSection = $('.fp-section.active');
    			var slides = activeSection.find('.fp-slides');
    			var activeSlide = slides.find('.fp-slide.active');
                
              //if you are on the first slide and scroll up
              if( activeSection.hasClass('move-section-up') && type == 'up' ){
                    var prev = $('.fp-section.active').prev('.fp-section');
        			if (prev.length) {
        				scrollPage(prev, null, true);
        			}
                }
                //if you are on the last slide and scroll down
                if( activeSection.hasClass('move-section-down') && type == 'down' ){
        			var next = $('.fp-section.active').next('.fp-section');
        			if(next.length){
        				scrollPage(next, null, false);
        			}
		...
		*/
		function performMovement(v){
			...
			if (options.css3 && options.autoScrolling && !options.scrollBar) {
                ...
                if( $('body').hasClass('fp-viewing-footer') ){
                    //calculate current translate value and add footer height
                    //to stop translate of second to last slide - check global.js
                    var height = $('.section:first-of-type').height() * ($('.section').size() - 2);
                    var translateAmt = height + 320; 
                    translate3d = 'translate3d(0px, -' + translateAmt + 'px, 0px)';
                }
                else{
		...
		*/
		function responsive(){
			if(options.responsive){
				...
				if( $(window).width() < options.responsive || $(window).height() < 650 ){
					if(!isResponsive){
      			...
      			$.fn.fullpage.setAllowScrolling(false);
					...
					addMouseWheelHandler(); //reinit this bc it gets stuck
