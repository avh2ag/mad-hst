...
(function (factory) {
	...
	$.BigVideo = function(options, element) {
		...
		var defaults = {
			...
			controls:false,
			doLoop:true,
			container: element ? $(element) : $('.video-wrap'),
			shrinkable:false,
			ambient: true,
			loop: true,
		};
			...
			if (windowAspect < mediaAspect) {
				...
				if (currMediaType === 'video') {
					...
					container.find(vidEl+'_html5_api')
						...
						.css('height',windowH);
					container.find(vidEl+'_flash_api')
						.css('width',windowH*mediaAspect)
						.css('height',windowH);
				} else {
					// is image
		...
		BigVideo.init = function() {
			if (!isInitialized) {
				...
				player = $('<video class="'+vidEl.substr(1)+'" class="video-js vjs-default-skin" preload="auto" loop="true" data-setup="{}" '+autoPlayString+' webkit-playsinline playsinline muted></video>');
				...
				container.find('.big-video-vid_flash_api')
					.attr('scale','noborder')
					.attr('width','100%')
					.attr('height','100%');
				...
				player.on('loadedmetadata', function(data) {
					if (container.find('.big-video-vid_flash_api').length) {
						// use flash callback to get mediaAspect ratio
						mediaAspect = 
							container.find('.big-video-vid_flash_api')[0].vjs_getProperty('videoWidth') /
							container.find('.big-video-vid_flash_api')[0].vjs_getProperty('videoHeight');
					} else {
						// use html5 player to get mediaAspect
						mediaAspect =
							container.find('.big-video-vid_html5_api').prop('videoWidth') /
							container.find('.big-video-vid_html5_api').prop('videoHeight');
					}
					updateSize();
